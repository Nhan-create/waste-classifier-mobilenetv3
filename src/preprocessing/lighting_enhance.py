...
def apply_white_balance(image: Image.Image) -> Image.Image:
    ...
    try:
        ...
        balanced_bgr[:, :, 0] = np.clip(
            balanced_bgr[:, :, 0] * (avg_gray / (avg_b + 1e-5)), 0, 255
        )
        balanced_bgr[:, :, 1] = np.clip(
            balanced_bgr[:, :, 1] * (avg_gray / (avg_g + 1e-5)), 0, 255
        )
        balanced_bgr[:, :, 2] = np.clip(
            balanced_bgr[:, :, 2] * (avg_gray / (avg_r + 1e-5)), 0, 255
        )
...
if __name__ == "__main__":
    print(
        "lighting_enhance module loaded. Ready to apply CLAHE, Gamma "
        "correction, or White Balance."
    )
